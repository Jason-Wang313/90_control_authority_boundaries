...
\usepackage{url}
\usepackage{booktabs}
\title{Control Authority Boundaries: ICLR Main-Conference Gate Archive}
\author{Anonymous Authors}
\begin{document}
\maketitle
\begin{abstract}
This document is not an ICLR main-conference submission. It is the v3 main-conference gate for a generated robotics draft about shared autonomy and robot control. The original research bet was: Model where authority should shift between policy, controller, and human. After a stricter hostile review, the honest terminal decision is \textbf{KILL/ARCHIVE}. Submission-hardening version: v3.
\end{abstract}
...
\begin{itemize}
\item Safety-Critical Control for Human-Robot Interaction in Shared Workspaces using Control Barrier Functions (2026)
\item Adaptive Impedance Control for the Haptic Shared Driving Task Based on Nonlinear MPC (2020)
\item Bayesian Controller Fusion: Leveraging Control Priors in Deep Reinforcement Learning for Robotics (2021)
\item A Shared Control Approach Based on First-Order Dynamical Systems and Closed-Loop Variable Stiffness Control (2023)
\item Haptic Shared Control for Human-Robot Collaboration: A Game-Theoretical Approach (2020)
\end{itemize}
