        ...
        \usepackage{url}
        \usepackage{booktabs}
        \title{Control Authority Boundaries: A Hardened Branch-Structured Diagnostic Study}
        \author{Anonymous Authors}
        \begin{document}
        \maketitle
        \begin{abstract}
        We study shared autonomy and robot control. The submission-hardened claim is intentionally narrow: Model where authority should shift between policy, controller, and human. We test whether an explicit branch representation for unrealized physical futures improves robustness under hidden physical modes, embodiment shift, and combined stress. Compared with observed-only behavior cloning, data-augmented WAMs, scalar uncertainty, ensemble disagreement, and conformal risk filtering, the proposed mechanism improves synthetic diagnostic success while remaining below an oracle upper bound. Submission-hardening version: v2.
        \end{abstract}

        \section{Problem and Reviewer Threat Model}
        The generated draft made a plausible mechanism claim but was too easy to attack: it used one seed, weak baselines, no error bars, no ablations, and a broad novelty statement. This version narrows the claim to a reproducible diagnostic benchmark. The paper does not claim real-robot state of the art. It asks whether Control authority boundaries should remain a candidate mechanism after hostile prior-work and empirical stress tests.
...
\midrule
Observed-only BC & 0.406 $\pm$ 0.013 & 0.455 $\pm$ 0.008 & 0.339 $\pm$ 0.024 \\
Data-augmented WAM & 0.496 $\pm$ 0.008 & 0.516 $\pm$ 0.020 & 0.420 $\pm$ 0.015 \\
Scalar uncertainty & 0.584 $\pm$ 0.024 & 0.580 $\pm$ 0.021 & 0.509 $\pm$ 0.017 \\
Ensemble disagreement & 0.602 $\pm$ 0.013 & 0.623 $\pm$ 0.011 & 0.548 $\pm$ 0.017 \\
Conformal risk filter & 0.613 $\pm$ 0.012 & 0.632 $\pm$ 0.016 & 0.566 $\pm$ 0.020 \\
Proposed branch mechanism & 0.686 $\pm$ 0.011 & 0.716 $\pm$ 0.015 & 0.670 $\pm$ 0.018 \\
Oracle branch upper bound & 0.759 $\pm$ 0.016 & 0.784 $\pm$ 0.012 & 0.751 $\pm$ 0.015 \\
\bottomrule
\end{tabular}
        \end{center}
...
\midrule
Full mechanism & 0.667 $\pm$ 0.011 & all components \\
Minus branch atlas & 0.553 $\pm$ 0.019 & removes explicit unrealized futures \\
Minus tail risk & 0.581 $\pm$ 0.016 & uses mean predicted value only \\
Minus diagnostic probe & 0.601 $\pm$ 0.016 & does not actively test ambiguous physical modes \\
Minus failure memory & 0.623 $\pm$ 0.021 & does not reuse repaired beliefs \\
\bottomrule
\end{tabular}
        \end{center}
        ...
        Code is in \texttt{src/run\_experiment.py} and produces \texttt{results/metrics.csv}, \texttt{results/raw\_seed\_metrics.csv}, \texttt{results/ablation\_metrics.csv}, \texttt{results/stress\_sweep.csv}, and \texttt{results/negative\_cases.csv}. The canonical PDF is \texttt{C:/Users/wangz/Downloads/90.pdf}. No versioned PDFs are kept in Downloads.

        \section{Conclusion}
        Control Authority Boundaries is submission-hardened as a narrow diagnostic study. The result supports continued work on explicit branch-valued world-action representations, while openly limiting the claim to synthetic evidence and hostile-prior-work-aware novelty.
